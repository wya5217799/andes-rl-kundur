...
objective: >-
  Revise the ICEMS paper after the negative R291 Gate A: retain fixed 3 s only
  as a bounded benchmark, state that the tested deterministic handoff did not
  solve fast--slow coupling, and constrain the centralized-versus-shared-MARL
  conclusion to the executed scalar-action formulation.
decision_refs:
