"""Fully normalized minimum-norm residual solver for R348."""
...
def solve_fully_normalized_minimum_norm_edge_residual(
    *,
    base_outputs: object,
    ...
    """Solve the unchanged residual problem in dimensionless coordinates."""
    ...
    target_slack = endpoint_constraints_original(relaxed_scaled_edges)
    ...
    if not target_feasible:
        edges, residual_nodes, total_commands, counterfactual, soc_path = decode_scaled(
        ...
        return ResidualSolveResult(
            ...
            message=f"fully normalized relaxation: {relaxed_result.message}",
        )

    def objective(scaled_values: np.ndarray) -> float:
        return float(scaled_values @ scaled_values)

    def gradient(scaled_values: np.ndarray) -> np.ndarray:
        return 2.0 * scaled_values
    ...
    edges, residual_nodes, total_commands, counterfactual, soc_path = decode_scaled(scaled_values)
    endpoint_slack = endpoint_constraints_original(scaled_values)
    physical_slack = physical_constraints_original(scaled_values)
    ...
    return ResidualSolveResult(
        ...
        message=(
            f"fully normalized relaxation: {relaxed_result.message}; minimum norm: {result.message}"
        ),
    )
