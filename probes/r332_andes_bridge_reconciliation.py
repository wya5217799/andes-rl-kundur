"""Fail-closed R332 static ANDES bridge reconciliation classifier."""
...
def _json_pointer_resolves(path: Path, fragment: str) -> bool:
...
def _implementation_locator_resolves(locator: str) -> bool:
    file_part, separator, fragment = locator.partition("#")
    line_number: int | None = None
    candidate, line_separator, suffix = file_part.rpartition(":")
    if line_separator and suffix.isdigit():
        file_part = candidate
    ...
    if not resolved.is_file():
        return False
    if separator:
        return resolved.suffix.lower() == ".json" and _json_pointer_resolves(resolved, fragment)
...
def _mapping_fields_and_locators_valid(rows: list[Mapping[str, Any]]) -> bool:
    for row in rows:
        ...
        if str(row.get("disposition")) not in ALLOWED_DISPOSITIONS:
            return False
        implementation = row.get("implementation_locators")
        official = row.get("official_locators")
        if not _is_locator_list(implementation) or not _is_locator_list(official):
            return False
        if not all(_implementation_locator_resolves(str(locator)) for locator in implementation):
            return False
        if not all(_official_locator_is_versioned_primary(str(locator)) for locator in official):
            return False
        if not all(
...
def evaluate_bridge_reconciliation(payload: object) -> dict[str, Any]:
    """Return the R332 platform judgment and every explicit failure."""
