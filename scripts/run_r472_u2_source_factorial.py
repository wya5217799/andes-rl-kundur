"""R472 U2 successor: immutable R471 reuse plus detached orchestration."""
...
_spec = importlib.util.spec_from_file_location(
    "_r472_r470_core", ROOT / "scripts/run_r470_u2_source_factorial.py"
)
...
ROUND_ID = "R472"
PLAN = ROOT / "memory/rounds/R472/plan.md"
LINE = ROOT / "paper/yang_md_decoupling_marl/LINE.md"
POWER = ROOT / "memory/rounds/R472/power_analysis.json"
REUSE = ROOT / "memory/rounds/R472/reuse_audit.json"
CAPACITY = ROOT / "memory/rounds/R472/capacity_evidence.json"
REHEARSAL = ROOT / "memory/rounds/R472/rehearsal.json"
SEAL = ROOT / "memory/rounds/R472/formal_seal.json"
OUT = ROOT / "results/research_loop/r472_u2_source_factorial"
R471_OUT = ROOT / "results/research_loop/r471_u2_source_factorial"
TRAIN_SHARDS = ROOT / "tmp/andes/r472_train_shards.json"
EVAL_SHARDS = ROOT / "tmp/andes/r472_eval_shards.json"

EXPECTED_REUSE = tuple(
    [("a0_c0_r0", seed) for seed in core.TRAINING_SEEDS]
    + [("a0_c0_r1", seed) for seed in core.TRAINING_SEEDS]
    + [("a0_cp_r0", seed) for seed in (401, 402, 403, 404)]
)
...
def build_contract() -> dict[str, Any]:
    ...
    inherited["operational_change"] = "detached WSL pipeline outside unified execution session"
    contract["r472"] = inherited
    return contract
...
def authority_checks() -> dict[str, bool]:
    ...
    return {
        "active_plan": "round: R472" in plan and "state: active" in plan,
        "active_line": "line_id: yang-md-decoupling-marl" in line and "status: active" in line,
        "contract_closed": len(core.ARMS) == 18 and len(core.TRAINING_SEEDS) == 6 and len(EXPECTED_REUSE) == 16,
...
def reuse_audit() -> dict[str, Any]:
    if OUT.exists() or REHEARSAL.exists() or SEAL.exists():
        raise FileExistsError("reuse audit must precede R472 network/formal artifacts")
    donor_entries, errors = _verify_hashed_tree(R471_OUT / "donors")
    donor_rows: dict[int, dict[str, Any]] = {}
    for seed in core.TRAINING_SEEDS:
        manifest_path = R471_OUT / "donors" / f"seed{seed}" / "manifest.json"
    ...
    for arm, seed in EXPECTED_REUSE:
        run_dir = R471_OUT / "train" / arm / f"seed{seed}"
    ...
    partial_dirs = sorted(
        core._relative(path)
        for path in (R471_OUT / "train").glob("*/*")
    ...
    return {
        ...
        "errors": errors,
        "passed": not errors and len(completed) == 16,
    }
...
def measure_capacity() -> dict[str, Any]:
    ...
    payload["other_python_processes"] = [
        ...
        if "run_r472_u2_source_factorial.py capacity" not in row
    ]
...
def rehearsal() -> dict[str, Any]:
    ...
    checks["terminal_truth_table"] = {
        ...
        "source": "R471 sealed terminal predicate; R472 performs no donor regeneration",
    }
    ...
    with tempfile.TemporaryDirectory(dir=Path.cwd()) as folder:
        ...
        source.write_bytes(b"r472-hardlink-probe")
    ...
    checks["passed"] = bool(
        ...
        and checks["reuse_audit"]["completed_count"] == 16
...
def import_parent_artifacts() -> str:
    ...
    core.load_seal()
    reuse = core._read_hashed_json(REUSE)
    if not reuse["passed"] or int(reuse["completed_count"]) != 16:
        raise RuntimeError("reuse audit is not valid")
    if OUT.exists():
        raise FileExistsError(f"R472 output exists: {OUT}")
    entries = _hardlink_tree(R471_OUT / "donors", OUT / "donors")
    for arm, seed in EXPECTED_REUSE:
        entries.extend(
            _hardlink_tree(
                R471_OUT / "train" / arm / f"seed{seed}",
...
def prepare() -> dict[str, Any]:
    ...
    sources = {
        ...
        "successor_tests": ROOT / "tests/test_run_r472_u2_source_factorial.py",
        "detached_pipeline": ROOT / "scripts/run_r472_detached_pipeline.sh",
