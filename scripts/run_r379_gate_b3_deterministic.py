#!/usr/bin/env python3
"""Rehearse, seal, and execute the create-only R379 Gate B-3 deterministic bank.

Usage (WSL):
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r379_gate_b3_deterministic.py rehearse
    /home/wya/andes_venv/bin/python scripts/run_r379_gate_b3_deterministic.py prepare
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r379_gate_b3_deterministic.py execute \
...
distributed arms use the low-corner high-pass mutual-damping successor law.
"""
...
ROUND_ID = "R379"
ROUND_DIR = ROOT / "memory/rounds/R379"
...
DEFAULT_OUT = ROOT / "results/research_loop/r379_gate_b3_deterministic"
...
def _source_paths() -> dict[str, Path]:
    return {
        ...
        "gate_b3_contract": (
            ROOT
            / "paper/paralleled_vsg_marl/working/"
            "gate_b3_deterministic_physical_contract.md"
        ),
        "runner": Path(__file__).resolve(),
        "runner_tests": ROOT / "tests/test_r379_gate_b3_deterministic.py",
        "controller": (
            ROOT
        ...
        "classifier_tests": (
            ROOT / "tests/test_gate_b3_deterministic.py"
        ),
...
def _parent_paths() -> dict[str, Path]:
    return {
        ...
        "r377_capacity": ROOT / "memory/rounds/R377/capacity_evidence.json",
        "r377_analysis": (
            ROOT
            / "results/research_loop/r377_gate_b2_deterministic/"
...
def _plan_is_active() -> bool:
    ...
    return "round: R379" in text and "state: active" in text
...
def _contract_is_closed(contract: Mapping[str, Any]) -> bool:
    try:
        return bool(
            ...
            and float(contract["highpass_alpha"]) == 0.90
...
def rehearse() -> tuple[str, str]:
    ...
    checks = {
        ...
        "parent_sidecars": all(
            ...
            for path in (
                ROOT / "memory/rounds/R377/formal_seal.json",
                ROOT / "memory/rounds/R377/capacity_evidence.json",
                ROOT
                / "results/research_loop/r377_gate_b2_deterministic/"
                "development_analysis.json",
                ROOT
