"""Sealed WSL runner for R418 (feedback loop / objective-repair round: B1 bundle plus the frozen R403-style action-effort term in the CD differential channel): slew-state bundle.
...
Lifecycle (WSL only, always through the scratch launcher):
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py measure-capacity
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py rehearse
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py prepare
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py train --arm <arm> --seed <seed>
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py evaluate
  python scripts/andes_scratch.py scripts/run_r420_objective_repair.py classify

All formal artifacts are create-only with sha256 sidecars under
results/research_loop/r420_objective_repair/.
"""
...
ROUND_ID = "R420"
PLAN = ROOT / "memory/rounds/R420/plan.md"
LINE = ROOT / "paper/yang_md_decoupling_marl/LINE.md"
REHEARSAL = ROOT / "memory/rounds/R420/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R420/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R420/formal_seal.json"
OUT = ROOT / "results/research_loop/r420_objective_repair"
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R420 physical commands are WSL/POSIX-only")
...
def load_seal() -> dict[str, Any]:
    ...
    for name, entry in (seal.get("sources") or {}).items():
        if entry["sha256"] != _sha256_file(ROOT / entry["path"]):
            raise RuntimeError(f"source drifted from the R420 seal: {name}")
    return seal
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r420_objective_repair.py",
...
def _other_processes() -> list[dict[str, Any]]:
    ...
    for entry in _other_research_python_processes():
        ...
        if "run_r420_objective_repair.py" in command:
            continue
        ...
        for _step_index in range(steps_per_episode):
            ...
            if arm_id == "yang_scalar_td3":
                ...
                if tds_failed:
                    ...
                    # R420 single factor: the CD differential channel gains
                    # the frozen R403-style action-effort term (weight 1.0)
                    # over the per-step mean squared executed action norm.
                    differential, common, _effort = (
                        physical_costs_with_action_effort(
                            frequencies[None, :],
                            rocof[None, :],
                            np.asarray(info["P_es"], dtype=float)[None, :],
                            action.astype(float)[None, :, :],
                            contract=contract,
                            action_weight=ACTION_EFFORT_WEIGHT,
                        )
                    )
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R420 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
        raise FileExistsError("R420 formal output exists before capacity")
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R420 rehearsal checks failed: " + str(checks))
    ...
    try:
        ...
        for arm_id in contract["learning_arm_ids"]:
            ...
            next_joint = _joint_obs(observation)
            # R420-abort lesson: the rehearsal must exercise the replay
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R420 authority checks failed: " + str(checks))
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R420 capacity gate is not RUN-READY")
    if not _plan_process_budget_matches(capacity):
        raise RuntimeError("R420 plan does not freeze the measured process budget")
    for payload in (rehearsal, capacity):
        if payload["sources"] != snapshot_sources:
            raise RuntimeError("R420 source drift before seal")
    ...
    if SEAL.exists() or OUT.exists():
        raise FileExistsError("R420 formal artifact exists before sealing")
    ...
    return _write_new_json(
        SEAL,
        {
            ...
            "single_factor_change": (
                "CD objective action-effort term: the CD differential "
                "channel gains the frozen R403-style per-step mean squared "
                "executed-action-norm term with weight 1.0; the slew-state "
...
def main() -> int:
    ...
    if args.command == "measure-capacity":
        safe_emit(f"R420 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R420 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R420 formal seal: {prepare()}")
        ...
        safe_emit(
            "R420 training manifest: "
            + train_arm_seed(arm, seed, restart_count=args.restart_count)
        )
    elif args.command == "evaluate":
        evaluate_all()
        safe_emit("R420 evaluation complete")
    else:
        safe_emit(f"R420 formal analysis: {classify()}")
    return 0
