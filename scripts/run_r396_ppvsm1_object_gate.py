"""Run the third science-identical correction of the invalid PPVSM1 gate.

Motivation:
    R395 is analysis-invalid by CLM-1120: the initial trace row stored
    device-major dictionaries while the capture-trace consumer reads the
    signal-major shape. R396 repairs only that seam and runs the complete
    scientific arm inside rehearsal so every record step executes before the
    seal.

Usage:
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r396_ppvsm1_object_gate.py rehearse
    /home/wya/andes_venv/bin/python scripts/run_r396_ppvsm1_object_gate.py prepare
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r396_ppvsm1_object_gate.py execute --expected-seal-sha256 <sha256>
...
PARENT_RUNNER = ROOT / "scripts/run_r395_ppvsm1_object_gate.py"
_parent_spec = importlib.util.spec_from_file_location("r396_r395_parent", PARENT_RUNNER)
if _parent_spec is None or _parent_spec.loader is None:  # pragma: no cover
    raise RuntimeError(f"cannot load R395 parent runner: {PARENT_RUNNER}")
...
PLAN = ROOT / "memory/rounds/R396/plan.md"
QUESTION = ROOT / "memory/questions/Q-0110.md"
REHEARSAL = ROOT / "memory/rounds/R396/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R396/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R396/formal_seal.json"
DEFAULT_OUT = ROOT / "results/research_loop/r396_ppvsm1_object_gate"

R395_PARENT_SHA256 = {
    "seal": "f693ad656041dcff6c742de4a25ba83b1fd59c06e4d054c9da1b47cfb2414ad1",
    "attempt": "c2ecfeabb401ac5fbb0ee26b8df53f62e9dfe1d6de434173152529161cb58051",
    "execution": "24809114d87cd53e95d5893d0b802df2f6a53a7cc5d4fc72bcae554000328ead",
    "analysis": "be209cb548352aa17aaa69608a2462db8ad0c61649f8ba5bb14bff10c8e6889a",
    "manifest": "bd41198d5157411b778301ee7e514d1b067a293878b1a40441de7e76626c8162",
    "claim": "a2d0a801ab2056954359f823f3c781eb8f491ea63fc118f9b1fba8197e64b544",
    "feed": "bfd25b81b654f89ac0ea72ffd33475968e1d9a299f1d75b22a41069bfc4cb6a2",
    "verdict": "0d37311d8bd8665c08d9ce28a2670b7c3c67e0907a2fee38b91053497151cd08",
...
def build_r396_contract() -> dict[str, Any]:
    """Return R393's science unchanged plus R396 correction provenance."""
    ...
    contract.update(
        {
            ...
            "parent_r395_sha256": deepcopy(R395_PARENT_SHA256),
            "evidence_corrections": ["signal_major_initial_trace_shape"],
        }
    )
    return contract
...
def source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_r396_ppvsm1_object_gate.py",
        "plan": PLAN,
        "question": QUESTION,
...
def parent_manifest() -> dict[str, dict[str, str]]:
    result_root = ROOT / "results/research_loop/r395_ppvsm1_object_gate"
    parents = {
        ...
        "claim": ROOT / "memory/claims/CLM-1120.md",
        "feed": ROOT / "paper/converter_vsg_pq_decoupling/reports/R395.md",
        "verdict": ROOT / "memory/rounds/R395/verdict.md",
        "r391_analysis": ROOT
...
def validate_r395_parent_chain(contract: Mapping[str, Any]) -> bool:
    try:
        ...
        return bool(
            seal["round"] == "R395"
            and attempt["round"] == "R395"
            and execution["round"] == "R395"
            and manifest["round"] == "R395"
...
def _initial_trace_row(system: Any) -> dict[str, Any]:
    """Signal-major initial trace row matching the capture-trace consumer."""
    ...
    for signal, variable in variable_map.items():
        ...
        devices[signal] = {
            str(idx): float(values[int(variable.a[position])])
...
def _run_arm(
    ...
    """R395's arm with only the signal-major trace-shape correction."""

    record = parent_runner.parent_runner.parent_runner._empty_record(
        payload_sha256(contract)
    )
    ...
    try:
        ...
        record["inventory"] = parent_runner.parent_runner.parent_runner._inventory(
        ...
        if not (
            ...
            record["finite_guard"] = (
                parent_runner.parent_runner.parent_runner._finite_status(
        ...
        system.j_update(models=models, info="R396 fixed equilibrium EIG snapshot")
        ...
        if not eig_return:
            ...
            return record
        state_matrix = (
            parent_runner.parent_runner.parent_runner._dense_state_matrix(system)
        )
        ...
        if not np.all(np.isfinite(eigenvalues.real)) or not np.all(
            ...
            raise RuntimeError("R396 spectrum is not finite")
        record["spectrum"] = {
            "captured": True,
        ...
        record["trace"] = (
            parent_runner.parent_runner.parent_runner._capture_trace(
                system, initial
            )
        )
        record["finite_guard"] = (
...
def setup_only_canary(runtime: Mapping[str, Any]) -> dict[str, Any]:
    ...
    contract = build_r396_contract()
    record = _run_arm(contract, runtime)
    classification = classify_ppvsm1_object_record(record, contract)
    result.update(
        {
            "full_arm_canary": {
                "classification": classification["classification"],
                "execution_error": record["execution_error"],
                "scientific_error": record["scientific_error"],
                "terminal_time_seconds": record["solver"][
                    "terminal_time_seconds"
                ],
                "trace_checked": record["trace"]["checked"],
                "trace_times": len(record["trace"]["times"]),
                "reference_deviations": [
                    row["abs_deviation"] for row in record["references"]["rows"]
                ],
                "spectrum_summary": classification.get("spectrum_summary", {}),
            },
        }
    )
    result["runtime_api_present"] = bool(
        result["runtime_api_present"]
        and classification["classification"] == "PPVSM1-OBJECT-PASS"
    )
    return result
...
def _configure_parent() -> None:
    ...
    parent_runner.build_r395_contract = build_r396_contract
    parent_runner.validate_r394_parent_chain = validate_r395_parent_chain
...
def rehearse() -> str:
    contract = build_r396_contract()
    if not validate_r395_parent_chain(contract):
        raise RuntimeError("R396 frozen R395 parent chain failed before rehearsal")
...
def prepare() -> str:
    contract = build_r396_contract()
    if not validate_r395_parent_chain(contract):
        raise RuntimeError("R396 frozen R395 parent chain failed before sealing")
