...
write_inventory() {
  local exit_code="$1"
  R476_PIPELINE_PHASE="${phase}" R476_PIPELINE_EXIT_CODE="${exit_code}" \
    "${python_bin}" scripts/andes_scratch.py \
      scripts/run_r476_u2_confirmatory.py inventory >/dev/null 2>&1 || true
}
...
runner=("${python_bin}" scripts/andes_scratch.py scripts/run_r476_u2_confirmatory.py)
...
for wave in 1 2 3; do
  run_phase "training-wave-${wave}" "${driver[@]}" \
    ...
    --shards "tmp/andes/r476_train_wave${wave}_shards.json" \
    --workers 16 \
    --round R476 \
    --log-dir "tmp/andes/r476_train_logs/wave${wave}"
  if [[ "${wave}" == "1" ]]; then
    first_wave_result="$(find tmp/andes/r476_train_logs/wave1 -name driver_result.json -type f | sort | tail -n 1)"
...
run_phase training-completeness "${python_bin}" -c \
  'from pathlib import Path; import json; paths=list(Path("results/research_loop/r476_u2_confirmatory/train").glob("*/*/manifest.json")); assert len(paths)==48, f"expected 48 training manifests, found {len(paths)}"; assert all(json.load(open(path))["valid"] and int(json.load(open(path))["interaction_steps"])==43200 for path in paths)'
