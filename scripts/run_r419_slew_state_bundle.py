"""Sealed WSL runner for R418 (feedback loop / program B1, successor of the aborted R418 run with the flattened store and the store/update rehearsal fix): slew-state bundle.
...
Lifecycle (WSL only, always through the scratch launcher):
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py measure-capacity
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py rehearse
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py prepare
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py train --arm <arm> --seed <seed>
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py evaluate
  python scripts/andes_scratch.py scripts/run_r419_slew_state_bundle.py classify
...
ROUND_ID = "R419"
PLAN = ROOT / "memory/rounds/R419/plan.md"
LINE = ROOT / "paper/yang_md_decoupling_marl/LINE.md"
REHEARSAL = ROOT / "memory/rounds/R419/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R419/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R419/formal_seal.json"
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R419 physical commands are WSL/POSIX-only")
...
def load_seal() -> dict[str, Any]:
    ...
    for name, entry in (seal.get("sources") or {}).items():
        if entry["sha256"] != _sha256_file(ROOT / entry["path"]):
            raise RuntimeError(f"source drifted from the R419 seal: {name}")
    return seal
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r419_slew_state_bundle.py",
...
def _other_processes() -> list[dict[str, Any]]:
    ...
    for entry in _other_research_python_processes():
        ...
        if "run_r419_slew_state_bundle.py" in command:
            continue
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R419 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
        raise FileExistsError("R419 formal output exists before capacity")
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R419 rehearsal checks failed: " + str(checks))
    ...
    try:
        ...
        for arm_id in contract["learning_arm_ids"]:
            ...
            next_joint = _joint_obs(observation)
            # R419-abort lesson: the rehearsal must exercise the replay
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R419 authority checks failed: " + str(checks))
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R419 capacity gate is not RUN-READY")
    if not _plan_process_budget_matches(capacity):
        raise RuntimeError("R419 plan does not freeze the measured process budget")
    for payload in (rehearsal, capacity):
        if payload["sources"] != snapshot_sources:
            raise RuntimeError("R419 source drift before seal")
    ...
    if SEAL.exists() or OUT.exists():
        raise FileExistsError("R419 formal artifact exists before sealing")
...
def main() -> int:
    ...
    if args.command == "measure-capacity":
        safe_emit(f"R419 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R419 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R419 formal seal: {prepare()}")
        ...
        safe_emit(
            "R419 training manifest: "
            + train_arm_seed(arm, seed, restart_count=args.restart_count)
        )
    elif args.command == "evaluate":
        evaluate_all()
        safe_emit("R419 evaluation complete")
    else:
        safe_emit(f"R419 formal analysis: {classify()}")
    return 0
