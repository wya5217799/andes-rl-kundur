#!/usr/bin/env python3
"""Versioned R292 formal adapter after the q0-screen implementation repair.

The numerical runner, seven-arm matrix, endpoints, guards, classification, and
20,000-resample analysis remain in ``run_r292_formal.py``.  This adapter only
binds them to the versioned, trace-reuse screen and records the execution
amendment in a new formal seal.  The failed first screen and formal-prepare log
remain untouched.
"""
...
SPEC = importlib.util.spec_from_file_location("run_r292_formal_core", CORE_PATH)
...
FORMAL.FRESH_DIR = ROOT / "results/r292_fresh_bank_v2"
...
FORMAL.DEFAULT_SEAL = ROOT / "memory/rounds/R292/formal_v2_seal.json"
FORMAL.DEFAULT_OUT = ROOT / "results/r292_formal_evaluation_v2"
...
def _source_paths() -> dict[str, Path]:
    return {
        ...
        "formal_v2_adapter": Path(__file__).resolve(),
        "recovery_launcher": ROOT / "scripts/run_r292_recovery_unattended.sh",
        "vector_runner": ROOT
...
def prepare(manifest_path: Path, out_dir: Path) -> None:
    ...
    if manifest_path.exists():
        raise FileExistsError(f"v2 formal seal already exists: {manifest_path}")
    if trace_dir.exists() and any(trace_dir.glob("*.json")):
        raise ValueError("v2 formal seal must precede every v2 controller trace")
    ...
    if provenance.get("performance_endpoints_inspected") is not False:
        raise ValueError("v2 screen inspected forbidden performance endpoints")
    if provenance.get("andes_trajectory_count") != 0:
        raise ValueError("v2 screen unexpectedly ran new ANDES trajectories")
    for path_text, digest in provenance["trace_hashes"].items():
        if FORMAL.sha256_file(ROOT / path_text) != digest:
            raise ValueError(f"v2 screen trace drift: {path_text}")
    bank, bank_hash = FORMAL.load_scenario_bank(
        FORMAL.FORMAL_BANK,
        expected_sha256=screen["formal_bank_sha256"],
    )
    if bank["scenario_count"] != 24:
        raise ValueError("R292 v2 formal bank must contain exactly 24 scenarios")
    ...
    payload: dict[str, Any] = {
        ...
        "recovery": {
            ...
            "first_invalid_screen_preserved": True,
