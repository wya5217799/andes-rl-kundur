"""Sealed WSL runner for R415 (soft-spot program A4, successor of the aborted R414 run with the canonical-blocks comparison fix applied): energy-port extra banks.
...
Lifecycle (WSL only, always through the scratch launcher):
  python scripts/andes_scratch.py scripts/run_r415_energy_port_extra_banks.py measure-capacity
  python scripts/andes_scratch.py scripts/run_r415_energy_port_extra_banks.py rehearse
  python scripts/andes_scratch.py scripts/run_r415_energy_port_extra_banks.py prepare
  python scripts/andes_scratch.py scripts/run_r415_energy_port_extra_banks.py execute
  python scripts/andes_scratch.py scripts/run_r415_energy_port_extra_banks.py classify

All formal artifacts are create-only with sha256 sidecars under
results/research_loop/r415_energy_port_extra_banks/.
"""
...
PLAN = ROOT / "memory/rounds/R415/plan.md"
REHEARSAL = ROOT / "memory/rounds/R415/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R415/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R415/formal_seal.json"
OUT = ROOT / "results/research_loop/r415_energy_port_extra_banks"
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R415 physical commands are WSL/POSIX-only")
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r415_energy_port_extra_banks.py",
        "banks": ROOT
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R415 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
        raise FileExistsError("R415 formal output exists before capacity")
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R415 rehearsal checks failed: " + str(checks))
    ...
    if not record["completed_steps"] or record["tds_failed"]:
        raise RuntimeError("R415 rehearsal trajectory failed")
    return _write_new_json(
        REHEARSAL,
        {
            "schema_version": 1,
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R415 authority checks failed: " + str(checks))
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R415 capacity gate is not RUN-READY")
    if not _plan_process_budget_matches(capacity):
        raise RuntimeError("R415 plan does not freeze the serial process budget")
    for payload in (rehearsal, capacity):
        if payload["sources"] != snapshot_sources:
            raise RuntimeError("R415 source drift before seal")
        if payload["installed_runtime"] != snapshot_runtime:
            raise RuntimeError("R415 runtime drift before seal")
    if rehearsal["parents"] != snapshot_parents:
        raise RuntimeError("R415 parent drift before seal")
    if SEAL.exists() or OUT.exists():
        raise FileExistsError("R415 formal artifact exists before sealing")
    return _write_new_json(
        SEAL,
        {
            "schema_version": 1,
...
def load_seal() -> dict[str, Any]:
    ...
    if seal.get("blocks") != _blocks_canonical():
        raise RuntimeError("frozen banks drifted from the R415 seal")
    for name, entry in (seal.get("sources") or {}).items():
        if entry["sha256"] != _sha256_file(ROOT / entry["path"]):
            raise RuntimeError(f"source drifted from the R415 seal: {name}")
    return seal
...
def execute() -> str:
    ...
    if not checks["output_absence"]:
        raise FileExistsError("R415 formal output root already exists")
...
def main() -> int:
    ...
    if args.command == "measure-capacity":
        safe_emit(f"R415 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R415 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R415 formal seal: {prepare()}")
    elif args.command == "execute":
        safe_emit(f"R415 formal attempt: {execute()}")
    else:
        safe_emit(f"R415 formal analysis: {classify()}")
    return 0
