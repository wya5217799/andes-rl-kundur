"""R56 — score TD3+LSTM h=64 norm 3-seed sweep + diagnostic.

Mirrors :mod:`_r51_score_sac_h64`, adapted for TD3LSTMAgent ckpts
(``algo='td3_lstm'`` is auto-detected by ``checkpoint_loader.load_agents``).
Outputs JSON with both 6-axis scores and the temporal-variance
diagnostic metrics (``dM_span``, ``dD_span``, cross-agent ``corr``)
that the R49–R55 hexagon used to characterise the static-setpoint
attractor.

Success criteria (CLM-0063 candidate):
- PRIMARY (H1):  3-seed mean 6-axis > 0.40
- SECONDARY (H2): per-agent dM_span > 30 % of paper
- FLOOR (H3):    mean ∈ [0.30, 0.40] AND dM_span > 25 %
- NULL (H4):     mean < 0.20 OR seed collapse

Run: /home/wya/andes_venv/bin/python scripts/_r56_score_lstm.py
"""
...
SEEDS = (49, 50, 51)
HIDDEN = (64,)  # single LSTM cell width


def main() -> None:
    rows = []
    for seed in SEEDS:
        ckpt = ROOT / "results" / f"td3_lstm_h64_s{seed}"
        label = f"td3_lstm_h64_s{seed}"
    ...
    print()
    print("=== R56-α TD3+LSTM h=64 norm 3-seed sweep ===")
    print(
        f"  6-axis: mean={sum(sc) / len(sc):.4f}  range=[{min(sc):.4f}, {max(sc):.4f}]"
    )
    print()
    print("Reference points (production triangle + hexagon):")
    print("  R48-β TD3 norm h=64 (production single):    0.334  dD_sp~12%")
    print("  R48-δ HAWE h=64 median (production ensemble): 0.351")
    print("  R48-β s51 single (strongest non-lucky):     0.365")
    print("  R49-α R03 obs (FAIL):                       0.263  dD_sp~4.6%")
    print("  R50-α LAMBDA=-100 W=1 (FAIL):               0.110  dD_sp~0.3%")
    print("  R51-α SAC h=64 (FAIL):                      0.107")
    print("  R55-α LAMBDA=-100 W=10 (FAIL):              0.110  dD_sp~0.3%")
    print()
    print(f"  R56-α: dH_util_mean={util_h_mean:.3f} dD_util_mean={util_d_mean:.3f}")
    print(f"  R56-α: dM_span_mean={span_m:.1f}% dD_span_mean={span_d:.1f}%")

    summary = {
        "label": "r56_alpha_lstm",
        "rows": rows,
    ...
    out = ROOT / "results" / "research_loop" / "r56_alpha_lstm.json"
