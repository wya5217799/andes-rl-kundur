#!/usr/bin/env python3
"""Seal, run, evaluate, and analyse the R308 strict-TDS canary.

``prepare``, ``eval``, and ``analyse`` are import-safe on Windows. ``run``
imports ANDES only inside the command and must be launched through the WSL
scratch runner. Every formal JSON artifact is create-only and sidecar-bound.
"""
...
from andes_rl_kundur.evaluation.model_first_tds_canary import (  # noqa: E402
    ACTIVE_STEPS,
...
ROUND_ID = "R308"
QUESTION_ID = "Q-0064"
PLAN = ROOT / "memory/rounds/R308/plan.md"
QUESTION = ROOT / "memory/questions/Q-0064.md"
DEFAULT_SEAL = ROOT / "memory/rounds/R308/tds_canary_seal.json"
DEFAULT_OUT = ROOT / "results/r308_model_first_tds_canary"
...
def _source_paths() -> dict[str, Path]:
    return {
        ...
        "evaluator": SRC
        / "andes_rl_kundur/evaluation/model_first_tds_canary.py",
        "contract_tests": ROOT / "tests/test_model_first_contract.py",
        "evaluator_tests": ROOT / "tests/test_model_first_tds_canary.py",
        "adapter_tests": ROOT / "tests/test_r308_model_first_tds_canary.py",
    }
...
def build_contract() -> dict[str, Any]:
    return {
        ...
        "stage": "solver-readback-diagnosis-only",
        "tds_method": "trapezoid",
        "tds_stopping_semantics": "max_abs_newton_correction",
        "tds_convergence_tolerance": EXPECTED_TOLERANCE,
        "tds_tiny_correction_threshold": EXPECTED_TOL_ZERO,
...
def _load_seal(path: Path, expected: str) -> tuple[dict[str, Any], str]:
    ...
    if seal.get("round") != ROUND_ID or seal.get("question") != QUESTION_ID:
        raise RuntimeError("R308 seal identity mismatch")
    if seal.get("contract_payload_sha256") != _payload_sha256(seal["contract"]):
        raise RuntimeError("R308 seal contract payload drift")
    if seal["contract"].get("training_authorized") is not False:
        raise RuntimeError("R308 seal unexpectedly authorizes training")
...
def _run_trace(*, coordinate: str, sign: str, seal_digest: str) -> dict[str, Any]:
    ...
    return {
        ...
        "guards": {"tds_test_ok": tds_test_ok},
...
def run(seal_path: Path, expected: str, out_dir: Path) -> None:
    ...
    if manifest_path.exists():
        raise FileExistsError(f"R308 run already exists: {manifest_path}")
...
def _load_run_records(
    ...
    if manifest.get("seal_sha256") != seal_digest:
        raise RuntimeError("R308 manifest seal mismatch")
    records = [
...
def eval_records(seal_path: Path, expected: str, out_dir: Path) -> None:
    ...
    decision = evaluate_tds_canary_records(records)
    scorecard = {
        "schema_version": 1,
        "round": ROUND_ID,
        "question": QUESTION_ID,
        ...
        "evidence_status": {
            ...
            "eligible": None,
            "scope": "solver-readback-diagnosis-only",
        },
        **decision,
    }
...
def analyse(seal_path: Path, expected: str, out_dir: Path) -> None:
    ...
    if scorecard.get("classification") != decision["classification"]:
        raise RuntimeError("R308 scorecard decision drift")
    analysis = {
        "schema_version": 1,
        "round": ROUND_ID,
        "question": QUESTION_ID,
