#!/usr/bin/env python3
"""Rehearse, seal, and execute the create-only R377 Gate B-2 deterministic bank.

Usage (WSL):
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r377_gate_b2_deterministic.py rehearse
    /home/wya/andes_venv/bin/python scripts/run_r377_gate_b2_deterministic.py prepare
    /home/wya/andes_venv/bin/python scripts/andes_scratch.py \
        scripts/run_r377_gate_b2_deterministic.py execute \
...
outer VSG energy-port projection must be identity on every step.  The
distributed arms use the high-pass filtered mutual-damping successor law.
"""
...
ROUND_ID = "R377"
ROUND_DIR = ROOT / "memory/rounds/R377"
...
DEFAULT_OUT = ROOT / "results/research_loop/r377_gate_b2_deterministic"
...
def _source_paths() -> dict[str, Path]:
    return {
        ...
        "runner_tests": ROOT / "tests/test_r377_gate_b2_deterministic.py",
        "controller": (
            ROOT
        ...
        "controller_tests": (
            ROOT / "tests/test_feasibility_native_deterministic.py"
        ),
        "action_map": (
            ROOT
        ...
        "classifier_tests": (
            ROOT / "tests/test_gate_b2_deterministic.py"
        ),
...
def _source_manifest() -> dict[str, dict[str, str]]:
    ...
    for name, path in _source_paths().items():
        if not path.is_file():
            raise FileNotFoundError(f"missing R377 source: {path}")
...
def _parent_paths() -> dict[str, Path]:
    return {
        "r376_seal": ROOT / "memory/rounds/R376/formal_seal.json",
        "r376_capacity": ROOT / "memory/rounds/R376/capacity_evidence.json",
        "r376_analysis": (
            ROOT
            / "results/research_loop/r376_gate_b_deterministic/"
            "development_analysis.json"
        ),
        "r376_feed": ROOT / "paper/paralleled_vsg_marl/reports/R376.md",
        "r376_verdict": ROOT / "memory/rounds/R376/verdict.md",
        "r376_claim": ROOT / "memory/claims/CLM-1025.md",
        "r374_development": (
            ROOT
...
def _parent_manifest() -> dict[str, dict[str, str]]:
    ...
    for name, path in _parent_paths().items():
        if not path.is_file():
            raise FileNotFoundError(f"missing R377 parent: {path}")
        result[name] = {"path": _relative(path), "sha256": _sha256_file(path)}
    return result
...
def _plan_is_active() -> bool:
    ...
    return "round: R377" in text and "state: active" in text
...
def _contract_is_closed(contract: Mapping[str, Any]) -> bool:
    try:
        return bool(
            ...
            and float(contract["highpass_alpha"]) == 0.60
    ...
    *,
    ...
    return {
        ...
        "empirical_anchor": {
            ...
            "capacity_path": parents["r375_capacity"]["path"],
            "capacity_sha256": parents["r375_capacity"]["sha256"],
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R377 physical/rehearsal commands are WSL/POSIX-only")
    if Path.cwd().resolve() == ROOT.resolve():
        raise RuntimeError("R377 must run through scripts/andes_scratch.py")
...
def rehearse() -> tuple[str, str]:
    ...
    if collisions:
        raise FileExistsError(f"R377 readiness output collision: {collisions}")
    ...
    checks = {
        ...
        "parent_sidecars": all(
            ...
            for path in (
                ROOT / "memory/rounds/R376/formal_seal.json",
                ROOT / "memory/rounds/R376/capacity_evidence.json",
                ROOT
                / "results/research_loop/r376_gate_b_deterministic/"
                "development_analysis.json",
                ROOT
    ...
    if payload["readiness"] != "RUN-READY":
        raise RuntimeError(f"R377 rehearsal HOLD: {checks}")
...
def prepare() -> str:
    if SEAL.exists() or DEFAULT_OUT.exists():
        raise FileExistsError("R377 seal or formal output already exists")
    ...
    if rehearsal.get("readiness") != "RUN-READY" or not _rehearsal_checks(rehearsal):
        raise RuntimeError("R377 rehearsal is not RUN-READY")
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R377 capacity is not RUN-READY")
    sources = _source_manifest()
    parents = _parent_manifest()
    if sources != rehearsal["sources"] or parents != rehearsal["parents"]:
        raise RuntimeError("R377 source or parent drift after rehearsal")
    contract = build_contract()
    if _payload_sha256(contract) != rehearsal["contract_sha256"]:
        raise RuntimeError("R377 contract drift after rehearsal")
    payload = {
        "schema_version": 1,
        "round": ROUND_ID,
        "created_utc": datetime.now(UTC).isoformat(),
...
def execute(*, expected_sha256: str) -> str:
    ...
    if DEFAULT_OUT.exists():
        raise FileExistsError(f"R377 output collision: {DEFAULT_OUT}")
...
def _load_seal(expected_sha256: str) -> tuple[dict[str, Any], str]:
    ...
    if digest != expected_sha256:
        raise RuntimeError("R377 expected seal hash does not match")
    if seal.get("contract_sha256") != _payload_sha256(seal["contract"]):
        raise RuntimeError("R377 sealed contract hash mismatch")
    if seal.get("sources") != _source_manifest():
        raise RuntimeError("R377 source drift after seal")
    if seal.get("parents") != _parent_manifest():
        raise RuntimeError("R377 parent drift after seal")
    if seal.get("rehearsal_sha256") != _sha256_file(REHEARSAL):
        raise RuntimeError("R377 rehearsal drift after seal")
    if seal.get("capacity_sha256") != _sha256_file(CAPACITY):
        raise RuntimeError("R377 capacity drift after seal")
    if not _contract_is_closed(seal["contract"]):
        raise RuntimeError("R377 sealed contract is not closed")
    return seal, digest
