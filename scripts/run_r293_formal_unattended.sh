...
SEAL="$ROOT/memory/rounds/R293/formal_seal.json"
OUT="$ROOT/results/r293_formal_evaluation"
LOGS="$OUT/logs"
STATUS="$OUT/status"
mkdir -p "$LOGS" "$STATUS"
if [[ -e "$STATUS/complete" || -e "$STATUS/failed" ]]; then
  echo "R293 formal evaluation already has terminal status" >&2
  exit 2
fi
expected="$(awk 'NR==1 {print $1}' "$SEAL.sha256")"
if [[ -z "$expected" ]]; then
  echo "missing R293 formal seal hash" >&2
  exit 2
fi
...
for shard in 0 1 2; do
  "$PYTHON" "$ROOT/scripts/andes_scratch.py" \
    "$ROOT/scripts/run_r293_formal.py" run \
...
"$PYTHON" "$ROOT/scripts/andes_scratch.py" \
  "$ROOT/scripts/run_r293_formal.py" analyse \
  --expected-manifest-sha256 "$expected" \
