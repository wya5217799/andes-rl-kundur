"""Rehearse, seal, and execute the create-only R361 message-extension gate.

The adapter rebuilds the R359 exact fifteen-field causal observations,
attaches the frozen one-hop neighbour messages (four fields per neighbour
node, two neighbours per edge, twenty-three fields total), fits the four
pre-registered frozen non-neural map families (fixed affine, RBF kernel
ridge, k-NN, quadratic polynomial basis) per physical edge, and evaluates
development leave-one-scenario-out per family.  Any passing family yields the
registered four-way OR classification; a failure of every family further
weakens the surveyed information-path hypothesis and stops this route.  A
failed development gate stops before the frozen holdout controller, holdout
counterfactuals, simulation, or training.
"""
...
from probes.r361_neighbour_message_residual import (  # noqa: E402
    build_message_observations_from_parent_inventory,
...
PLAN = ROOT / "memory/rounds/R361/plan.md"
QUESTION = ROOT / "memory/questions/Q-0098.md"
CAPACITY = ROOT / "memory/rounds/R361/capacity_evidence.json"
DEFAULT_REHEARSAL = ROOT / "memory/rounds/R361/rehearsal.json"
DEFAULT_SEAL = ROOT / "memory/rounds/R361/analysis_seal.json"
DEFAULT_OUT = ROOT / "results/r361_neighbour_message_residual"
...
def build_contract() -> dict[str, Any]:
    """Return the prospectively frozen message-extended analysis contract."""

    return {
        "schema_version": 1,
        "round": ROUND_ID,
        "question": QUESTION_ID,
        "stage": "message-extended-neighbour-learnability-gate",
        "inventory": {
        ...
        "information": {
            "edge_actor_count": 3,
            "continuous_fields_per_actor": MESSAGE_EXTENDED_OBSERVATION_DIMENSION,
            "own_fields": 15,
            "neighbour_message_fields": NEIGHBOUR_MESSAGE_DIMENSION,
            "neighbours_per_edge": 2,
            "one_hop_neighbour_table": {
                str(edge): list(message) for edge, message in ONE_HOP_NEIGHBOUR_MESSAGES.items()
            },
            "message_fields": ["frequency_deviation_hz", "rocof_hz_s", "soc", "voltage_pu"],
        ...
        "controller_family": {
            "members": sorted(MESSAGE_CONTROLLER_FAMILY),
...
def source_paths(*, include_rehearsal: bool) -> dict[str, Path]:
    """Return the complete R361 implementation closure."""
    ...
    paths = {
        ...
        "probe": ROOT / "probes/r361_neighbour_message_residual.py",
        "probe_tests": ROOT / "tests/test_r361_neighbour_message_residual.py",
        "controller_src": ROOT / "src/andes_rl_kundur/control/neighbour_message_residual.py",
        "adapter_tests": ROOT / "tests/test_r361_neighbour_message_residual_analysis.py",
...
def parent_paths() -> dict[str, Path]:
    """Return exact R341/R352 trace parents plus terminal R358-R360 evidence."""
...
def _verify_entry_preconditions(
    ...
    if out_dir.exists():
        raise FileExistsError(f"R361 result root already exists: {out_dir}")
    if "state: active" not in PLAN.read_text(encoding="utf-8"):
        raise RuntimeError("R361 active plan identity is missing")
    question_text = QUESTION.read_text(encoding="utf-8")
    if "status: in-flight" not in question_text or "R361:" not in question_text:
        raise RuntimeError("R361 in-flight question identity is missing")
    for path in source_paths(include_rehearsal=False).values():
        if not path.is_file():
            raise RuntimeError(f"R361 source is missing: {path}")
    ...
    if len(development) != 16 or len(holdout) != 16:
        raise RuntimeError("R361 requires sixteen exact pairs per bank")
...
def _observations(
    ...
    return build_message_observations_from_parent_inventory(
        inventory=inventory,
...
def _development_stage(
    ...
    integrity = {
        ...
        "neighbour_table_frozen": all(
            ...
            and all(
                row.edge == edge
                and row.source_neighbour.node_id == ONE_HOP_NEIGHBOUR_MESSAGES[edge][0]
                and row.target_neighbour.node_id == ONE_HOP_NEIGHBOUR_MESSAGES[edge][1]
    ...
    decision = classify_message_gate(
...
def _fit_full_development_family(
    *,
    ...
    fitter = MESSAGE_CONTROLLER_FAMILY[family_name]
    steps = int(horizon)
...
def rehearsal(
    ...
    *,
    ...
    if not (
        ...
        raise RuntimeError("R361 rehearsal inventory or observation check failed")
    ...
    return r353_parent._write_new_json(
        record_path,
        {
            ...
            "observation_fields_per_edge": MESSAGE_EXTENDED_OBSERVATION_DIMENSION,
            "observation_rows_per_edge": SAMPLES_PER_TRACE - STARTUP_ZERO_STEPS,
            "family_members": sorted(MESSAGE_CONTROLLER_FAMILY),
            "one_hop_neighbour_table": {
                str(edge): list(message) for edge, message in ONE_HOP_NEIGHBOUR_MESSAGES.items()
            },
            "holdout_residual_labels_read": 0,
...
def prepare(
    ...
    *,
    ...
    """Create the exact source-, parent-, and rehearsal-bound R361 seal."""
    ...
    if (
        ...
        or record.get("observation_fields_per_edge") != MESSAGE_EXTENDED_OBSERVATION_DIMENSION
        or record.get("family_members") != sorted(MESSAGE_CONTROLLER_FAMILY)
        ...
        raise RuntimeError("R361 rehearsal record drift")
    return r353_parent._write_new_json(
        seal_path,
        {
            "schema_version": 1,
...
def load_seal(
    ...
    *,
    ...
    """Verify the exact R361 seal and complete immutable closure."""

    payload = r353_parent._read_json(path)
    actual = r353_parent._sha256_file(path)
    if actual != expected_sha256:
        raise RuntimeError(f"R361 seal digest mismatch: {actual}")
    r353_parent._verify_sidecar(path)
    _verify_entry_preconditions(out_dir)
    ...
    if (
        ...
        raise RuntimeError("R361 contract, source, or parent drift")
...
def analyse(
    ...
    *,
    ...
    """Execute the single sealed staged offline R361 analysis."""
    ...
    try:
        ...
        analysis_digest = r353_parent._write_new_json(
            analysis_path,
            {
                ...
                "neighbour_message_constraints_included": True,
