...
ROUND_ID = "R424"
PLAN = ROOT / "memory/rounds/R424/plan.md"
LINE = ROOT / "paper/yang_md_decoupling_marl/LINE.md"
REHEARSAL = ROOT / "memory/rounds/R424/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R424/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R424/formal_seal.json"
OUT = ROOT / "results/research_loop/r424_guard_aligned_constraints"
...
# ── R424 is a solo round (no concurrent same-line round) ────────────────
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r424_guard_aligned_constraints.py",
        ...
        "repair_learner_tests": ROOT / "tests/test_cd_matd3_guard_constraints.py",
...
def _agent_for(arm_id: str, device: str) -> Any:
    ...
    if arm_id in ("cd_matd3_no_message", "cd_matd3_message"):
        # R424 single factor: the CD arms use the guard-aligned
        # action-constraint subclass; the reward seam, the Lagrange
        # machinery, and everything else are the R419 bundle verbatim.
        return GuardConstrainedSlewAwareCDMATD3(
...
def classify() -> str:
    ...
    analysis = {
        ...
        "repair": {
            "kind": "guard-aligned-action-constraints",
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R424 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R424 rehearsal checks failed: " + str(checks))
    ...
    scenario = profile["scenarios"][0]
    env = _build_env(profile)
    rehearsal_dir = ROOT / "tmp" / "andes" / "r424_rehearsal_checkpoints"
    ...
    try:
        ...
        for arm_id in contract["learning_arm_ids"]:
            ...
            if str(arm_id) != "yang_scalar_td3":
                ...
                # R424 rehearsal: exercise the guard-multiplier seam.
            ...
            if str(arm_id) in ("cd_matd3_no_message", "cd_matd3_message"):
                if not isinstance(agent, GuardConstrainedSlewAwareCDMATD3):
                    raise RuntimeError(
                        "rehearsal CD arm did not use the guard subclass"
                    )
                if not (
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R424 authority checks failed: " + str(checks))
...
def main() -> int:
    ...
    if args.command == "measure-capacity":
        safe_emit(f"R424 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R424 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R424 formal seal: {prepare()}")
        ...
        safe_emit(
            "R424 training manifest: "
            + train_arm_seed(arm, seed, restart_count=args.restart_count)
        )
    elif args.command == "evaluate":
        evaluate_all()
        safe_emit("R424 evaluation complete")
    else:
        safe_emit(f"R424 formal analysis: {classify()}")
    return 0
