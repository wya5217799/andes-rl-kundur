"""Sealed WSL runner for R412 (soft-spot program A2, successor of the aborted R412 run with the graceful EIG-failure fix applied): topology robustness.
...
Lifecycle (WSL only, always through the scratch launcher):
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py inventory
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py measure-capacity
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py rehearse
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py prepare
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py shards
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py shard <variant_id> [--resume]
  python scripts/andes_scratch.py scripts/run_r413_topology_robustness.py classify

All formal artifacts are create-only with sha256 sidecars under
results/research_loop/r413_topology_robustness/.
"""
...
PLAN = ROOT / "memory/rounds/R413/plan.md"
REHEARSAL = ROOT / "memory/rounds/R413/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R413/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R413/formal_seal.json"
OUT = ROOT / "results/research_loop/r413_topology_robustness"
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R413 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
        raise FileExistsError("R413 formal output exists before capacity")
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R413 physical commands are WSL/POSIX-only")
    if Path.cwd().resolve() == ROOT.resolve():
        raise RuntimeError("R413 must run through scripts/andes_scratch.py")
    import torch
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r413_topology_robustness.py",
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R413 rehearsal checks failed: " + str(checks))
    ...
    if not record["completed_steps"] or record["tds_failed"]:
        raise RuntimeError("R413 rehearsal trajectory failed")
    gate = eig_gate(variant)
    if not gate["passed"]:
        raise RuntimeError("R413 nominal EIG rehearsal gate failed")
    # R413-abort lesson: the rehearsal must also exercise the graceful
    ...
    for name, result in failure_path.items():
        if not isinstance(result.get("passed"), bool):
            raise RuntimeError(f"R413 rehearsal eig_gate malformed: {name}")
    return _write_new_json(
        REHEARSAL,
        {
            "schema_version": 1,
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R413 authority checks failed: " + str(checks))
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R413 capacity gate is not RUN-READY")
    if not _plan_process_budget_matches(capacity):
        raise RuntimeError("R413 plan does not freeze the measured process budget")
    for payload in (rehearsal, capacity):
        if payload["sources"] != snapshot_sources:
            raise RuntimeError("R413 source drift before seal")
    ...
    if SEAL.exists() or OUT.exists():
        raise FileExistsError("R413 formal artifact exists before sealing")
...
def main() -> int:
    ...
    if args.command == "inventory":
        ...
        safe_emit(f"R413 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R413 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R413 formal seal: {prepare()}")
        ...
        safe_emit(f"R413 shard complete: {args.args[0]}")
    else:
        safe_emit(f"R413 formal analysis: {classify()}")
    return 0
