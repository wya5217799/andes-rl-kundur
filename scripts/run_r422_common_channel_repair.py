"""Sealed WSL runner for the feedback-loop common-channel repair round (B1 bundle plus the frozen R403-style action-effort term in the CD COMMON channel).

Owner-authorized by the soft-spot program override deck item B1 (creative
mode, feedback-loop round).  The single registered factor versus R420 is
the channel placement of the frozen R403-style action-effort term (weight
1.0): R420 added the per-step mean squared executed-action norm to the CD
differential channel and measured a negative result (endpoint regression,
message increment flipped negative, no-harm guard profile worsened);
this round adds the SAME term to the CD common channel instead, leaving
the differential channel at the R419 term verbatim.  The R419 slew-state
bundle is otherwise unchanged: 9-slot prev-action augmentation with the
neighbour mask kept, aligned target/online post-projection semantics,
arms/seeds 401/402/403, 43,200 interaction steps per run, the same
hyperparameters, estimators, guards, and checkpoint rule.  The scalar arm
keeps its unchanged reward as the in-round isolation control.

Pre-registered decision rule: guards + endpoints versus R419/R420 and
versus the deterministic reference, plus the message contrast.  Creative
mode: any arm passing the physical guards or any classification flip is
recorded, the manuscript is updated per the evidence, and the loop
continues; no pause.
...
Lifecycle (WSL only, always through the scratch launcher):
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py measure-capacity
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py rehearse
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py prepare
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py train --arm <arm> --seed <seed>
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py evaluate
  python scripts/andes_scratch.py scripts/run_r422_common_channel_repair.py classify

All formal artifacts are create-only with sha256 sidecars under
results/research_loop/r422_common_channel_repair/.
"""
...
ROUND_ID = "R422"
PLAN = ROOT / "memory/rounds/R422/plan.md"
LINE = ROOT / "paper/yang_md_decoupling_marl/LINE.md"
REHEARSAL = ROOT / "memory/rounds/R422/rehearsal.json"
CAPACITY = ROOT / "memory/rounds/R422/capacity_evidence.json"
SEAL = ROOT / "memory/rounds/R422/formal_seal.json"
OUT = ROOT / "results/research_loop/r422_common_channel_repair"
...
ACTION_EFFORT_WEIGHT = 1.0  # frozen R403 repair weight

# ── owner-authorized concurrency (2026-08-17) ─────────────────────────
# R421 (B3 diagnostics) is in flight while this round trains.  Its live
# processes are declared here as the reserved share of the whole-host
# budget; the ladder is measured under that load and the memory rule is
# total-memory accounting (see the calibration log owner grant).
OTHER_RESERVED_PROCESSES = 7  # R421: 6 training workers + 1 driver
OTHER_RESERVED_RSS_BYTES = 7 * 950 * 1024**2  # measured ~930 MiB/worker
...
def _assert_wsl_scratch() -> None:
    if os.name != "posix":
        raise RuntimeError("R422 physical commands are WSL/POSIX-only")
...
def load_seal() -> dict[str, Any]:
    ...
    for name, entry in (seal.get("sources") or {}).items():
        if entry["sha256"] != _sha256_file(ROOT / entry["path"]):
            raise RuntimeError(f"source drifted from the R422 seal: {name}")
    return seal
...
def _source_manifest() -> dict[str, dict[str, str]]:
    sources = {
        ...
        "runner_tests": ROOT / "tests/test_run_r422_common_channel_repair.py",
...
def _other_processes() -> list[dict[str, Any]]:
    ...
    for entry in _other_research_python_processes():
        ...
        if "run_r422_common_channel_repair.py" in command:
            continue
        if "run_r421_diagnostics.py" in command:
            # owner-authorized concurrent round: R421 is the declared
...
def _common_channel_costs(
    ...
    """R422 reward seam: differential cost verbatim (R419), common cost
    gains the frozen R403-style action-effort term (weight 1.0).
        ...
        for _step_index in range(steps_per_episode):
            ...
            if arm_id == "yang_scalar_td3":
                ...
                if tds_failed:
                    ...
                    # R422 single factor: the CD common channel gains
...
def measure_capacity() -> str:
    ...
    for candidate in (CAPACITY, REHEARSAL, SEAL):
        if candidate.exists():
            raise FileExistsError(f"R422 pre-attempt artifact exists: {candidate}")
    if OUT.exists():
        raise FileExistsError("R422 formal output exists before capacity")
    ...
    return _write_new_json(
        CAPACITY,
        {
            ...
            "authorization": (
                "owner-authorized common-channel repair; ladder measured "
                "under the concurrent R421 load (declared reserved), "
...
def rehearse() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R422 rehearsal checks failed: " + str(checks))
    ...
    scenario = profile["scenarios"][0]
    env = _build_env(profile)
    rehearsal_dir = ROOT / "tmp" / "andes" / "r422_rehearsal_checkpoints"
    ...
    try:
        ...
        for arm_id in contract["learning_arm_ids"]:
            ...
            next_joint = _joint_obs(observation)
            # R422-abort lesson: the rehearsal must exercise the replay
...
def prepare() -> str:
    ...
    if not all(checks.get(key) is True for key in required):
        raise RuntimeError("R422 authority checks failed: " + str(checks))
    if capacity.get("readiness") != "RUN-READY":
        raise RuntimeError("R422 capacity gate is not RUN-READY")
    if not _plan_process_budget_matches(capacity):
        raise RuntimeError("R422 plan does not freeze the measured process budget")
    for payload in (rehearsal, capacity):
        if payload["sources"] != snapshot_sources:
            raise RuntimeError("R422 source drift before seal")
    ...
    if SEAL.exists() or OUT.exists():
        raise FileExistsError("R422 formal artifact exists before sealing")
    ...
    return _write_new_json(
        SEAL,
        {
            ...
            "single_factor_change": (
                "CD objective action-effort term, common channel: the CD "
                "common channel gains the frozen R403-style per-step mean "
                "squared executed-action-norm term with weight 1.0 (R420 "
                "negative placed it on the differential channel); the "
                "slew-state "
                "bundle (R419), arms, seeds, budgets, estimators, guards, "
                "and all other hyperparameters are the R419 assets verbatim; "
                "the scalar arm keeps its unchanged reward as the in-round "
                "contrast"
            ),
            "launch": {
...
def main() -> int:
    ...
    if args.command == "measure-capacity":
        safe_emit(f"R422 capacity evidence: {measure_capacity()}")
    elif args.command == "rehearse":
        safe_emit(f"R422 rehearsal artifact: {rehearse()}")
    elif args.command == "prepare":
        safe_emit(f"R422 formal seal: {prepare()}")
        ...
        safe_emit(
            "R422 training manifest: "
            + train_arm_seed(arm, seed, restart_count=args.restart_count)
        )
    elif args.command == "evaluate":
        evaluate_all()
        safe_emit("R422 evaluation complete")
    else:
        safe_emit(f"R422 formal analysis: {classify()}")
    return 0
