"""Directed tests for the R418 slew-state bundle runner.

Windows-safe: agent factory, observation augmentation flow, the median
endpoint table math, and the R410-formula message contrast.  The WSL-only
lifecycle (measure-capacity / rehearse / prepare / train / evaluate /
classify) runs through the scratch launcher in the sealed round itself.
"""
...
import run_r418_slew_state_bundle as runner  # noqa: E402
...
def test_message_improvement_median_then_ratio() -> None:
    # median-then-ratio (R410 endpoint-table formula) on synthetic medians
    medians = {
        "cd_matd3_no_message": {
            "off_diagonal_response_energy": 10.0,
