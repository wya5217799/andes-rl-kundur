"""Directed tests for the R425 sign-corrected guard-constraint runner.

Windows-safe: constants, the agent factory (CD arms -> signfix subclass,
scalar arm -> unchanged R419 class), the R419-verbatim reward seam, the
relative-residual math, the penalty-direction gate helper, and the
contract shape.  The WSL-only lifecycle runs through the scratch launcher
...
import run_r425_guard_constraints_signfix as runner  # noqa: E402
...
def test_agent_factory_mapping() -> None:
    ...
    assert isinstance(message, GuardConstrainedSlewAwareCDMATD3Signfix)
    no_message = runner._agent_for("cd_matd3_no_message", "cpu")
    assert isinstance(no_message, GuardConstrainedSlewAwareCDMATD3Signfix)
    scalar = runner._agent_for("yang_scalar_td3", "cpu")
    assert isinstance(scalar, SlewAwareYangScalarTD3)
    assert not isinstance(scalar, GuardConstrainedSlewAwareCDMATD3Signfix)
...
def test_penalty_direction_gate_helper() -> None:
    agent = runner._agent_for("cd_matd3_message", "cpu")
    agent.guard_multiplier_step(0.5, 0.5)  # mu > 0 for the probe
    runner._rehearsal_penalty_direction_check(agent)  # must not raise
