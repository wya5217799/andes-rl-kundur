"""Directed tests for the R415 energy-port extra-banks runner and banks.

Windows-safe: frozen bank validity, canonical JSON form, contract shape,
per-block decision math (stubbed summarizer), and serial capacity payload
fields.  The WSL-only lifecycle runs through the scratch launcher in the
sealed round itself.
"""
...
import run_r415_energy_port_extra_banks as runner  # noqa: E402
...
def test_blocks_canonical_json_round_trip() -> None:
    """R414-abort lesson (kept for provenance): the seal content and the
    load-time comparison must share one JSON-canonical form (tuples become
    lists)."""
...
def test_build_block_contract_shape() -> None:
    ...
    assert contract["r415"]["block_id"] == "a4_md_stiff"
    assert contract["r415"]["vsg_m0"] == 230.0
