...
- Phase-3B arm `an_cn_r1_rms`: authentic-source cell (actor N, critic N, reward access 1) with the reward replaced by the R433-frozen normalized action-RMS penalty formula; coefficients frozen (phi_abs 50.0, phi_d 0.0056, phi_f 100.0, phi_h 0.0056, delta_d_denominator 600.0, delta_m_denominator 600.0, action_penalty_mode normalized); no coefficient re-selection after result visibility; single change = penalty term (R424 target-semantics gate: rehearsal runs the gradient-direction probe; penalty lowers reward).
...
- Symmetry check (registered): if |mean - median|/SD > 1.0 on boundary-centred seed effects, the primary test switches to 1,000,000 fixed-seed sign-flip permutations (one-sided, p reported with its Monte Carlo SE); otherwise the exact Wilcoxon remains primary. Both reported.
