"""Triple-stack agent: WarmH0 actor + QR distributional critic + AFE critic input.

Combines three orthogonal interventions into a single TD3-LSTM variant:

1. **WarmH0 actor** (R96 / Q-0022 / CLM-0188): actor's LSTM ``(h_0, c_0)``
   are produced by a learned MLP head from the first-step obs. Eliminates
   the 10-step warm-up lag documented in CLM-0174.

2. **QR distributional critic** (R98 CLM-0157(a) / CLM-0189): critic emits
   N=51 quantile estimates instead of scalar Q; critic loss is quantile-
   Huber (Dabney 2018). Forces the critic to fit return *distribution*
   rather than just expectation.

3. **AFE critic input** (R98 CLM-0157(b) / CLM-0190): critic eats
   ``[obs, a, a², |a|, sign(a)]`` instead of ``[obs, a]``. Gives the
   bilinear LSTM-cell first layer a linear pathway to a² / |a| / sign(a)
   features — breaks the d²Q/da² ≈ 0 affine-Q pathology of CLM-0150.

If R122 (QR only), R123/R124 (AFE only), R127 (QR+AFE stacked) each give
partial lift over baseline 0.391, this triple-stack tests **additive**
mechanism intervention. The warm-h_0 + critic-rep fixes act at orthogonal
points: actor h_0 (CLM-0188) vs critic input dim (CLM-0190) vs critic
output head (CLM-0189), so composition is structurally clean.

R130+ launch path (when warmh0 + QR-AFE prototypes both pass single tests):

    LR=1e-4 python scripts/train.py --algo td3_warmh0_qr_afe_lstm \\
...
class TD3WarmH0QRAfeLstmAgent(TD3QRAfeLstmAgent):
    """Triple-stack: warm-h_0 actor + QR+AFE critic. Inherits all
    QR+AFE critic plumbing (quantile-Huber loss, AFE input expansion)
    from ``TD3QRAfeLstmAgent``; overrides only the actor + rollout
    init-hidden threading + actor warm-h_0 init in ``update``.
    """

    algo_name: str = "td3_warmh0_qr_afe_lstm"
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Build QR+AFE plumbing first (actor will be replaced next).
        super().__init__(
            obs_dim=obs_dim,
        ...
        # Replace the actor / actor_target with WarmH0 variant. Critic
        # stays the QR+AFE one already wired by the QR-AFE parent class.
        lr = self._target_lr
        hidden = self.hidden
    ...
    # ─── Update — warm h_0 actor + quantile-Huber critic ─────────────────

    def update(self) -> dict[str, Any] | None:
        """Override TD3QRLstmAgent.update to thread obs_for_warm into the
        actor / actor_target init_hidden at burn-in start. Everything
        else (quantile-Huber loss, AFE critic forward) comes from the
        QR+AFE parent's update via copy — we replicate the body but with
        warm-h_0 init for the actor.
        """
        ...
        # Quantile-Huber critic loss (identical to TD3QRLstmAgent.update)
        h_c1_critic = h_c1
        h_c2_critic = h_c2
