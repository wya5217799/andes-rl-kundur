"""Prospective R374 banks and physical cross-coordinate analysis."""
...
def _candidate_id(sync_gain: float, consensus_gain: float) -> str:
    return f"distributed_ks{sync_gain:g}_kc{consensus_gain:g}".replace(".", "p")


def _probe_arm_ids() -> list[str]:
    return [
        f"{mode}_{sign}"
        for mode in MODES
...
def build_contract() -> dict[str, Any]:
    """Return the JSON-safe frozen R374 comparison contract."""

    candidates = [
        {
            "arm_id": _candidate_id(sync_gain, consensus_gain),
            "sync_gain_system_pu_per_hz": sync_gain,
            "consensus_gain_per_s": consensus_gain,
        }
        for sync_gain in (0.5, 1.0)
        for consensus_gain in (0.5, 1.0)
    ]
    probe_arms = _probe_arm_ids()
    development_arms = [
        "zero_feedback",
        "local_diagonal_pi",
        *[str(candidate["arm_id"]) for candidate in candidates],
    ]
    evaluation_arms = [
        "zero_feedback",
        "local_diagonal_pi",
        "selected_distributed",
    ]
    return {
        "schema_version": 1,
        "round": "R374",
        "device_count": 4,
        "expected_vsg_idx": ["ES1", "ES2", "ES3", "ES4"],
        ...
        "modes": MODES,
        "mode_ids": list(MODES),
        "probe_component_magnitude_system_pu": 0.025,
        ...
        "development": {
            ...
            "disturbance_conditions": [
                {
                    "condition_id": "dev_disturbance_pq0_minus_0p75",
                    "delta_u": {"PQ_0": -0.75},
                },
                {
                    "condition_id": "dev_disturbance_pq1_plus_0p75",
                    "delta_u": {"PQ_1": 0.75},
                },
            ],
        ...
        "evaluation": {
            ...
            "disturbance_conditions": [
                {
                    "condition_id": "eval_disturbance_bus14_plus_0p85",
                    "delta_u": {"PQ_Bus14": 0.85},
                },
                {
                    "condition_id": "eval_disturbance_bus15_minus_0p85",
                    "delta_u": {"PQ_Bus15": -0.85},
                },
            ],
...
def probe_request(
    ...
    *,
    ...
    """Return one frozen additive probe vector."""
    ...
    return (
        direction
        * float(frozen["probe_component_magnitude_system_pu"])
...
def phase_jobs(
    ...
    *,
    ...
    if phase == "evaluation":
        ...
        arm_ids = [selected_arm_id if arm == "selected_distributed" else arm for arm in arm_ids]
...
def controller_spec(
    ...
    *,
    ...
    frozen = contract or build_contract()
    common = frozen["common_gains"]
    if arm_id == "zero_feedback":
        return {"architecture": "zero_feedback"}
    if arm_id == "local_diagonal_pi":
        return {
            "architecture": "local_diagonal_pi",
            "kp_system_pu_per_hz": float(common["kp_system_pu_per_hz"]),
            "ki_system_pu_per_hz_s": float(common["ki_system_pu_per_hz_s"]),
        }
    for candidate in frozen["distributed_candidates"]:
        if arm_id == candidate["arm_id"]:
            return {
                ...
                "sync_gain_system_pu_per_hz": float(
                    candidate["sync_gain_system_pu_per_hz"]
                ),
...
def _record_guard_errors(
    ...
    for record_index, record in enumerate(records):
        ...
        try:
            ...
            common = _step_matrix(record, "common_request_system_pu", columns=4)
            differential = _step_matrix(
                record, "differential_request_system_pu", columns=4
            )
            ...
            if np.max(np.abs(np.sum(differential, axis=1))) > zero_sum_atol:
                raise ValueError("differential request is not zero-sum")
            if not np.allclose(requested, common + differential, atol=atol, rtol=0.0):
                raise ValueError("controller channel reconstruction drift")
            if any(
                bool(reason)
                for row in rows
            ...
            residual_differential = residual - np.mean(residual, axis=1, keepdims=True)
...
def select_development_candidate(
    ...
    *,
    ...
    """Apply the frozen R374 development eligibility and ranking rule."""

    frozen = contract or build_contract()
    baseline = summaries.get("local_diagonal_pi")
    ...
    threshold = float(frozen["thresholds"]["development_primary_ratio_max"])
    ...
    for candidate_spec in frozen["distributed_candidates"]:
        ...
        if candidate is None or not bool(candidate.get("guards_pass")):
            continue
        offdiag_ratio, normalized_cross_ratio = _probe_ratios(candidate, baseline)
        ...
        if (
            offdiag_ratio <= threshold
            and normalized_cross_ratio <= threshold
            and differential_ratio <= threshold
            and common_ratio <= common_limit
        ):
            eligible.append(
                {
                    "arm_id": arm_id,
                    "off_diagonal_ratio": offdiag_ratio,
                    "normalized_cross_ratio": normalized_cross_ratio,
                    "differential_energy_ratio": differential_ratio,
                    "common_iae_ratio": common_ratio,
                    "rank_score": (
                        offdiag_ratio * normalized_cross_ratio * differential_ratio
                    ),
                    "sync_gain_system_pu_per_hz": float(
                        candidate_spec["sync_gain_system_pu_per_hz"]
                    ),
    ...
    eligible.sort(
        key=lambda row: (
            ...
            float(row["sync_gain_system_pu_per_hz"]),
...
def classify_summaries(
    ...
    *,
    ...
    try:
        ...
        baselines = [evaluation["zero_feedback"], evaluation["local_diagonal_pi"]]
    except KeyError:
        return {
            "classification": "ANALYSIS-INVALID",
            "checks": {},
    ...
    probe_ratio_rows = [_probe_ratios(selected, baseline) for baseline in baselines]
    cross_pass = all(
        offdiag <= primary_limit and normalized <= primary_limit
        for offdiag, normalized in probe_ratio_rows
    )
    ...
    settling_pass = (
        ...
        <= float(evaluation["local_diagonal_pi"]["disturbance"][
            "mean_differential_settling_seconds"
        ])
        - float(frozen["dt_seconds"])
        + 1.0e-12
    )
    differential_pass = (
    ...
    checks = {
        "cross_coordinate_decoupling": cross_pass,
        "differential_motion_and_settling": differential_pass,
    ...
    if not physical_pass:
        ...
        classification = "STOP-NO-CROSS-DECOUPLING"
    ...
    return {
        ...
        "checks": checks,
        "selected_arm_id": selected_id,
        "probe_ratios_vs_zero_and_diagonal": probe_ratio_rows,
        "differential_mean_ratios_vs_zero_and_diagonal": (
            differential_mean_ratios
        ),
        "training_authorized": False,
        "next_gate": (
