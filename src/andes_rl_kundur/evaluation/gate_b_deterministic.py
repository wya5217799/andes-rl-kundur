"""R376 Gate B frozen contract, guards, endpoints, and classification.

This module holds the byte-frozen R376 comparison contract from
``paper/paralleled_vsg_marl/working/gate_b_deterministic_physical_contract.md``
and the analysis used by the R376 runner.  It adapts the registered R375
endpoint definitions to the feasibility-native normalized-action arms; it is
not a copy of the frozen R374/R375 classifier, which remains immutable.
"""
...
SELECTED_ARM = "selected_distributed_feasibility_native"


def _candidate_id(sync_gain: float, consensus_gain: float) -> str:
    return (
        f"distributed_feasibility_native_ks{sync_gain:g}_kc{consensus_gain:g}"
    ).replace(".", "p")


def build_contract() -> dict[str, Any]:
    """Return the JSON-safe frozen R376 Gate B contract."""
    candidates = [
        {
    ...
    return {
        ...
        "development": {
            ...
            "probe_condition": {
                "condition_id": "dev_probe_pq15_minus_0p40",
                "delta_u": {"PQ_Bus15": -0.40},
            },
            "disturbance_conditions": [
                {
                    "condition_id": "dev_disturbance_pq0_plus_0p55",
                    "delta_u": {"PQ_0": 0.55},
                },
                {
                    "condition_id": "dev_disturbance_bus14_plus_0p50",
                    "delta_u": {"PQ_Bus14": 0.50},
                },
            ],
        ...
        "evaluation": {
            ...
            "probe_condition": {
                "condition_id": "eval_probe_pq1_plus_0p35",
                "delta_u": {"PQ_1": 0.35},
            },
            "disturbance_conditions": [
                {
                    "condition_id": "eval_disturbance_bus14_minus_0p45",
                    "delta_u": {"PQ_Bus14": -0.45},
                },
                {
                    "condition_id": "eval_disturbance_pq0_minus_0p65",
                    "delta_u": {"PQ_0": -0.65},
                },
            ],
...
def _record_guard_errors(
    ...
    for record_index, record in enumerate(records):
        ...
        try:
            ...
            common_action = _step_matrix(
                record, "common_action", columns=4
            )
...
def select_development_candidate(
    ...
    *,
    ...
    """Apply the frozen R376 development eligibility and ranking rule."""
    frozen = contract or build_contract()
...
def classify_summaries(
    ...
    *,
    ...
    return {
        ...
        "checks": checks,
        "selected_arm_id": selected_id,
        "probe_ratios_vs_zero_and_local": probe_ratio_rows,
